...
describe("Traversing DOM elements in Cypress", () => {
  ...
  it("children() to get the children of DOM elements", () => {
    ...
    cy.get('.traversal-breadcrumb').children().contains('Home').should('contain', 'Home');
  });
  ...
  it("first() to retrieve the first DOM element within elements ", () => {
   ...
   /*
   let arr = []
    cy.get('.traversal-table > tbody > tr > td').each(($el, index, $list) => {
      arr.push($el.text());
      cy.log(`Name ${$el.text()} added properly to the array`);
    });
